...
LRESULT CMainFrame::OnControlSelected(WPARAM wParam, LPARAM)
{
	CXTPControl* pControl = ((CXTPControl*)wParam);

	m_nIDControlTracking = pControl? pControl->GetID(): 0;
...
{
	if (m_nIDControlTracking > 0)
	{
		lParam = HID_BASE_COMMAND +  m_nIDControlTracking;
	}

	return CXTPMDIFrameWnd::OnCommandHelp(wParam, lParam);
}
