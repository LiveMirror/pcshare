...
int CMainFrame::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	CXTPToolBar* pPropertiesBar = (CXTPToolBar*)pCommandBars->Add(_T("Properties"), xtpBarTop);
	if (!pPropertiesBar || !pPropertiesBar->LoadToolBar(IDR_CHILDFRAME))
	{
		TRACE0("Failed to create toolbar\n");
		return -1;
	}

	pPropertiesBar->SetTitle("Propeties");

	DockRightOf(pPropertiesBar, pCommandBar);
...
void CMainFrame::OnCustomize()
{
	...
	pPageCommands->AddCategories(IDR_FLATTATYPE);

	// initialize the commands page page.
	pPageCommands->InsertAllCommandsCategory();
	pPageCommands->InsertBuiltInMenus(IDR_FLATTATYPE);
...
{
	if (lpCreateControl->nID == ID_FLAT_TAB_PROP)
	{
