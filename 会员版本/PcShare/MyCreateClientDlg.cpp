...
#ifndef _VIP_
	GetDlgItem(IDC_CHECK_ISSYS)->EnableWindow(FALSE);
	GetDlgItem(IDC_CHECK_ISKEY)->EnableWindow(FALSE);
	SetWindowText(_T("免费版本，只能生成4次"));
#endif

	return TRUE;  
}

BOOL CMyCreateClientDlg::AddDataToPe(BYTE* pSaveData, DWORD DataLen, BYTE* pPeData, DWORD nPeLen, LPCTSTR pPeFile)
{
	// PE头偏移
...
BOOL CMyCreateClientDlg::SaveFileData()
{
	...
	DWORD nSaveSize = m_DllSize + m_SysSize + m_ComSize + nInfoSize;
	BYTE* pTmpData = new BYTE[sizeof(MYSAVEFILEINFO) + nSaveSize];

	LPMYSAVEFILEINFO pSaveInfo = (LPMYSAVEFILEINFO) pTmpData;
	pSaveInfo->m_FindFlag = PS_VER_ULONGLONG;
	pSaveInfo->m_Size = nInfoSize;

	BYTE* pData = pTmpData + sizeof(MYSAVEFILEINFO);
	...
	if(!AddDataToPe(pTmpData, sizeof(MYSAVEFILEINFO) + nSaveSize, pExeFileData, m_ExeSize, szFile))
	{
...
void CMyCreateClientDlg::OnButtonCreate() 
{
	...
	//选择生成方式
	CMySelectTypeDlg SelectDlg(this);
...
#else
	SelectDlg.m_LoginType = 0;
#endif

	TCHAR m_FileName[256] = {0};
	if(SelectDlg.m_LoginType == 0) //服务器更新
	{
		//用户验证
		CMyLoginDlg LoginDlg(this, !SelectDlg.m_LoginType);
		if(LoginDlg.DoModal() != IDOK)
		{
			return;
		}

		BYTE* pFileData = (BYTE*) LoginDlg.pSystemFileData;
		if(pFileData == NULL)
		{
			MessageBox(_T("非法数据"), _T("错误"));
			return;
		}

		BYTE* pData = pFileData + sizeof(SYSFILEINFO);
		LPSYSFILEINFO pSysFileInfo = (LPSYSFILEINFO) pFileData;

		FCLzw lzw;

		//EXE数据
		lzw.PcUnZip(pData, &pExeFileData, &m_ExeSize);
		pData += pSysFileInfo->nExeFileLen;

		//DLL数据
		lzw.PcUnZip(pData, &pDllFileData, &m_DllSize);
		pData += pSysFileInfo->nDllFileLen;

		//SYS数据
		lzw.PcUnZip(pData, &pSysFileData, &m_SysSize);
		pData += pSysFileInfo->nSysFileLen;
		...
		COPYDATASTRUCT ct = {0};
		ct.lpData = pData;
		ct.cbData = pSysFileInfo->nModFileLen;
		ct.dwData = PS_LOCAL_UPDATE_MOD;
		if(!::SendMessage(hWnd, WM_COPYDATA, 0, (LPARAM) &ct))
		{
			return ;
		}
		pData += pSysFileInfo->nModFileLen;

#ifdef _VIP_
		//通知主界面更新插件数据
		ct.lpData = pData;
		ct.cbData = pSysFileInfo->nKeyFileLen;
		ct.dwData = PS_LOCAL_UPDATE_KEY;
		if(!::SendMessage(hWnd, WM_COPYDATA, 0, (LPARAM) &ct))
		{
			return ;
		}
