...
public :
	...
	BOOL PcZip(BYTE* pSrcData, BYTE* pDesData, DWORD* pDesLen);
...
inline void  FCLzw::Decode_InitStringTable () {
	FillMemory(m_pStrBegin, LZW_MAX_TABLE_SIZE * sizeof(LZW_STRING), 0xFF) ;
...
inline void  FCLzw::Encode_InitStringTable () {
	m_CurrTableIndex = m_LZW_END + 1 ;
	m_byCurrBits = m_byMinCode + 1 ;
	ZeroMemory (m_pHash, LZW_MAX_HASH_SIZE * sizeof(WORD)) ; // Hash Table置位0
}
