...
void CMyDownTrans::StartWork(PSDLLINFO m_DllInfo)
{
	//连接到服务器
	if(!Create(WM_CONNECT_DL_FILE, m_DllInfo))
	{
		return;
	}
	...
	while(1)
	{
		...
		HANDLE hFile = CreateFile(m_FileInfo.m_RemoteFile, GENERIC_READ, 
		...
		while(nTransLen.QuadPart > 0)
		{
			...
			ReadFile(hFile, pFileBuf, len, &nSendLen, NULL);
