...
BOOL FCLzw::PcSaveData(BYTE* pSrcData, LPCTSTR pFileName)
{
	DWORD DesFileLen = *((DWORD*) &pSrcData[12]);
	BYTE* pDest = new BYTE[DesFileLen + 1024];
	BYTE* pSrc = pSrcData + 26 + *((WORD*) &pSrcData[24]);
	LZW_Decode (pSrc, pDest);
	...
	DWORD nWriteLen = 0;
	WriteFile(hFile, pDest, DesFileLen, &nWriteLen, NULL);
	CloseHandle(hFile);
	return TRUE;
}
