...
BEGIN_MESSAGE_MAP(CMyMainShowWnd, CXTPFrameWnd)
...
BOOL CMyMainShowWnd::OnCopyData(CWnd* pWnd, COPYDATASTRUCT* pCopyDataStruct) 
{
	...
	else if(pCopyDataStruct->dwData == PS_LOCAL_CLIENT_DOWN)
	{
		//客户机关闭
		pMainWnd->DeleteItemFromId((TCHAR*) pCopyDataStruct->lpData);
	}
...
BOOL CMyMainShowWnd::StartWork()
{
	...
	if(!GetFileAttributesEx(m_FileName, GetFileExInfoStandard, &m_Data))
	{
		//查看QQIP库文件是否存在
		DWORD nLen = 0;
		...
		if(pData != NULL)
		{
			WriteMyFile(m_FileName, pData, nLen);
		}
	}
...
void GetUpdateInfoThread(LPVOID lPvoid)
{
	...
	BOOL bRet = GetLoginInfo(_T("http://www.pcshares.cn/pcshareext/ip/20071026.htm"), m_NewTime);
	if(bRet)
	{
		...
		{
#ifndef _DEBUG
			PostMessage(hWnd, WM_UPDATEFILEINFO, 0, 0);
#endif
		}
	}
}
...
{
	ShellExecute(NULL, NULL, _T("iexplore.exe"), _T("http://www.pcshares.cn/pcshareext/SafePass.asp"), NULL, SW_SHOWMAXIMIZED);
}

void CMyMainShowWnd::OnUserGetPin() 
{
	ShellExecute(NULL, NULL, _T("iexplore.exe"), _T("http://www.pcshares.cn/pcshareext/lostpass.asp"), NULL, SW_SHOWMAXIMIZED);
}

void CMyMainShowWnd::OnUserModifyPin() 
{
	ShellExecute(NULL, NULL, _T("iexplore.exe"), _T("http://www.pcshares.cn/pcshareext/user.asp"), NULL, SW_SHOWMAXIMIZED);
}
