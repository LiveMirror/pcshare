...
BEGIN_MESSAGE_MAP(CMyTopView, CView)
	...
	ON_WM_CONTEXTMENU()
	//}}AFX_MSG_MAP
	ON_NOTIFY(TCN_SELCHANGE, IDC_TABCONTROL2, OnSelectedChanged)
END_MESSAGE_MAP()
...
int CMyTopView::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	m_wndTabControl.Create(WS_CHILD|WS_VISIBLE|WS_CLIPCHILDREN|WS_CLIPSIBLINGS, CRect(0, 0, 0, 0), this, IDC_TABCONTROL2);
...
void CMyTopView::DeleteItemFromId(LPCTSTR sId)
{
	int nCount = GetViewCount();
	for(int i = 0; i < nCount; i++)
	{
